...
class KeyPointHead(nn.Module):
    def __init__(self, in_channels,num_keypoints=13, num_parts=6, upsample_ratio=4,output_keypoints=True):
        ...
        # 这些错误出现的地方都是通道不匹配
        #这里采用了硬编码
        self.feature_enhancement_modules = nn.ModuleList([FeatureEnhancementModule(40) for _ in range(num_parts)])
        ...
        #另一个和标准的不同的地方。
        self.channel_matching_conv = nn.Conv2d(in_channels, 40, kernel_size=1)
...
class FiniteNet(nn.Module):
    def __init__(self, num_keypoints=17, num_parts=6, pretrained=True, output_heatmaps=True):
        ...
        self.mobilenetv3 = models.mobilenet_v3_small(pretrained=pretrained)
        ...
        self.mid_level_features_index = 4
        self.high_level_features_index = 11
        self.keypoint_head = KeyPointHead(in_channels=96, num_keypoints=num_keypoints, num_parts=num_parts, output_keypoints= output_heatmaps)  # 假定高级特征层的通道数
        self.feature_fusion = FeatureFusion(low_channels=16, mid_channels=40, high_channels=96, enhanced_channels=40, attended_channels=40, out_channels=128)
        self.matting_head = MattingHead(in_channels=128)  # 假定融合特征有256个通道
