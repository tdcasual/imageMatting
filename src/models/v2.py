...
class ASPP(nn.Module):
    def __init__(self, in_channels, out_channels, output_stride=16):
        ...
        # Depthwise Separable Convolutions (integrated from AsppDepthwiseSeparableConv2d)
        self.aspp_depthwise_separable_convs = nn.ModuleList()
        for rate in dilation_rates:
            aspp_depthwise_separable_conv = nn.Sequential(
                nn.Conv2d(in_channels, in_channels, 3, padding=rate, dilation=rate, groups=in_channels, bias=False),
                nn.Conv2d(in_channels, out_channels, 1, bias=False),
                _layer_norm(out_channels),
                nn.ReLU(inplace=False))
            self.aspp_depthwise_separable_convs.append(aspp_depthwise_separable_conv)
        ...
        self.project = nn.Sequential(
            nn.Conv2d((len(dilation_rates) + 2) * out_channels, out_channels, 1, bias=False),
            _layer_norm(out_channels),
            nn.ReLU(inplace=False))

    def forward(self, x):
        res = [self.module_1x1(x)]
        
        # Apply integrated depthwise separable convolutions
        for conv_module in self.aspp_depthwise_separable_convs:
            res.append(conv_module(x))

        global_feat = self.global_avg_pool(x)
        global_feat_upsampled = F.interpolate(global_feat, size=x.size()[2:], mode='bilinear', align_corners=False)
        res.append(global_feat_upsampled)
...
class DepthwiseSeparableConv2d(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=1, dilation=1,activation=None):
        ...
        self.activate = nn.ReLU(inplace=False) if not activation else activation
...
class KeyPointHead(nn.Module):
    def __init__(self, in_channels,mid_channels,num_keypoints=13, num_parts=6, upsample_ratio=4,output_keypoints=True):
        ...
        self.attention_refine = nn.Sequential(
            DepthwiseSeparableConv2d(num_keypoints, 64, kernel_size=1, activation=nn.ReLU(inplace=False)),
...
class FiniteNet(nn.Module):
    def __init__(self, num_keypoints=17, num_parts=6, pretrained=True, output_heatmaps=True,fine_tune=False):
